...
## 2026-08-13 — Zoom Hard拡大率調整

- ZoomクイズのHard拡大率を3.6倍から4.5倍へ変更
- Hardの表示位置は中央35〜65%を避け、上下左右の外側からランダムに選択
- Easy 2.4倍、Normal 3.0倍は維持
- 難易度別の拡大率とHard表示位置のテストを更新

## 2026-08-13 — Skill Name Hard回答形式調整

- Skill NameのHardをChampion名の直接入力からスキル名の4択へ変更
- 問題のChampionとスキル枠を表示し、正解1つと編集距離が近いスキル名3つを提示
- Easy、Normalおよび他形式のHard回答は従来どおり維持

## 2026-08-13 — Skill Name全難易度の回答形式統一

- Skill NameのEasyとNormalもChampion名ではなくスキル名を選ぶ形式へ変更
- Champion画像と対象スキル枠を全難易度で表示
- Easyは正解1つ＋ランダムな実在スキル名3つ、Normalは正解1つ＋7つの8択
- Hardは正解1つ＋表記が似たスキル名3つの既存ルールを維持

## 2026-08-13 — Skill Name難易度別画像調整

- EasyはChampionアイコンと対象スキルアイコンを並べて表示
- Normalは対象スキルアイコンだけを表示
- HardはChampionアイコンと対象スキル枠の表示を維持

## 2026-08-13 — Skill Name設定画面文言調整

- 説明文を「チャンピオンのスキル名を当てる」へ変更
- Skill NameのHard回答形式表示を実際の動作に合わせて「入力」から「4択」へ変更

## 2026-08-13 — Skill Name Hard画像調整

- Hardの問題画像をChampionアイコン＋スキル枠からスキルアイコンだけの表示へ変更
- 正解1つ＋類似スキル名3つの4択ルールは維持

## 2026-08-13 — Skill Description回答形式調整

- 全難易度の選択肢をChampion名からスキル名へ変更
- Easyは4択、Normalは8択、Hardは4択で、誤答には実在する別スキル名を使用
- EasyとNormalの問題画面に対象スキルアイコンを表示
- Hardは説明文だけを手掛かりにスキル名を選ぶ

## 2026-08-13 — Skill Description問題表示再調整

- EasyとNormalに追加したスキルアイコン表示を撤回
- 全難易度で説明文だけを手掛かりにスキル名を選ぶ表示へ統一

## 2026-08-13 — Skill Description Hard逆引き形式

- Hardだけ出題方向を逆にし、スキルアイコンとスキル名から正しい説明文を選ぶ4択へ変更
- 正解1つと、他の実在スキル説明文3つをランダムに提示
- EasyとNormalは説明文からスキル名を選ぶ従来形式を維持
- 設定画面のHard回答形式表示を「入力」から「4択」へ変更

## 2026-08-13 — Voice音量設定

- Voice問題画面に0〜100%の音量スライダーを追加
- 再生中のPick/Ban音声へ音量変更を即時反映
- 選択音量をブラウザのlocalStorageへ保存し、次回アクセス時に復元
- 初回または保存値が無効な場合は70%を使用

## 2026-08-12 — Hardひらがな候補・正解判定

- ブラウザ標準datalistを独自候補リストへ変更し、日本語IME確定後も候補を確実に表示
- 候補クリックで入力欄へChampion名を反映
- Hardの正解判定でひらがな・カタカナを同一視
- 例: 「こぐ＝まう」「こぐ=まう」を「コグ＝マウ」の正解として判定

## 2026-08-12 — Hard入力時のIME重複修正

- 日本語IMEの変換中はChampion候補リストを更新しないよう変更
- 変換確定時に一度候補を更新し、先頭文字が二重入力される挙動を防止
- 英語入力時は従来どおり入力ごとに候補を更新

## 2026-08-12 — ZOOM表示位置調整

- EasyはSplash Artの中央を固定表示
- NormalとHardは問題生成ごとに縦横10〜90%の範囲から表示位置をランダム決定
- 難易度別の拡大率（Easy 2.4倍 / Normal 3倍 / Hard 3.6倍）は維持

## 2026-08-12 — Hard入力候補の途中入力対応

- 全クイズ共通のHard入力候補を部分一致で表示
- ひらがな入力をカタカナへ正規化し、「こぐ」から「コグ＝マウ」を検索可能に変更
- 日本語入力時は日本語名、英語入力時は英語名だけを表示する既存ルールを維持

## 2026-08-12 — ホーム画面コピー調整

- キャッチコピーを「己の知識で、試練に挑め」へ変更
- キャッチコピー下の説明文を削除
- クイズ一覧下の実装済み案内文を削除
...
## 2026-08-12 — ユーザー確認後の個別調整 1

### 共通

- HOMEカードとSkill Name / Descriptionのダイヤ内文字を逆回転し、枠に対して文字を正立表示
- 設定画面の「同じChampionは1セッション内で重複しません。」を削除

### Champion

- Hardの候補を入力文字種で分離
- 日本語入力時は日本語名のみ、半角英字入力時は英語名のみ最大12件表示
- 空入力時は候補を表示しない
...
- Easyの出題対象からPassive（P）を除外
- Normal / HardはP/Q/W/E/Rを維持

### 確認

- [x] 自動テスト28件成功
- [x] Zoom倍率 240% / 300% / 360%をブラウザ確認
- [x] 「ジ」入力で日本語候補のみ、「ji」入力で英語候補のみ表示
- [x] ダイヤ内文字の正立表示をPC画面で確認
- [x] 削除指定文言が画面に存在しないことを確認
- [x] ブラウザconsole errorなし
